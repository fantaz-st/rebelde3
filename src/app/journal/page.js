...
import { breadcrumb, JsonLd, SITE_URL, pageMetadata } from "@/lib/schema";
...
export async function generateMetadata() {
  return pageMetadata({
    path: "/journal",
    title: "Journal — Stories from the Adriatic",
    description: "Guides, observations, and behind-the-scenes notes from Rebelde Boats on private boat tours in Croatia.",
  });
}
...
export default async function JournalIndexPage({ params }) {
  ...
  const heroDesc  = "Guides, observations, and behind-the-scenes notes from Rebelde Boats on private boat tours in Croatia.";
  const readMoreLabel = "Read the story";

  const crumbsJsonLd = breadcrumb([{ name: "Journal", url: "/journal" }]);
