...
export default function BoatSectionItem({ item, index, isLast }) {
  const t  = useTranslations("boatSections");
  const ts = useTranslations(`boatSectionItems.${item.key}`);
  ...
  useParallaxImage(rootRef, {
    ...
    fromScale: 1.12, fromYPercent: -8,
    toScale: 1,      toYPercent: 6,
    ...
    () => {
      ...
      mm.add("(min-width: 768px)", () => {
        const tl = gsap.timeline({
          defaults: { ease: "none" },
          scrollTrigger: { trigger: hero, scroller, start: "top bottom+=200%", end: "bottom bottom", scrub: true, invalidateOnRefresh: true },
        });
        tl.fromTo(heroImg, { clipPath: `inset(14% 37.35% 14% 37.35% round ${r12}px)` }, { clipPath: "inset(0% 0% 0% 0% round 0px)" })
          .fromTo(heroTxt, { scale: 0.6, transformOrigin: "center" }, { scale: 1 }, 0);
      ...
      mm.add("(max-width: 767px)", () => {
        const tl = gsap.timeline({
          defaults: { ease: "none" },
          scrollTrigger: { trigger: hero, scroller, start: "top 90%", end: "top 40%", scrub: true, invalidateOnRefresh: true },
        });
        tl.fromTo(heroImg, { clipPath: `inset(25% 15% 25% 15% round ${r12}px)` }, { clipPath: "inset(0% 0% 0% 0% round 0px)" })
          .fromTo(heroTxt, { scale: 0.75, transformOrigin: "center" }, { scale: 1 }, 0);
        return () => { tl.scrollTrigger?.kill(); tl.kill(); };
      });
  ...
  useEffect(() => {
    ...
    if (!container) return;
    Fancybox.bind(container, `[data-fancybox="${galleryGroup}"]`, { Thumbs: { type: "modern" }, Carousel: { infinite: true } });
    ...
    <section id={item.key} className={classes.item} ref={rootRef}>
      ...
      <div ref={heroRef} className={classes.heroImg}>
        <div ref={heroImgRef} className={classes.heroImgInner}>
          <Image src={item.heroImg} alt="" fill priority={index === 0} sizes="100vw" className={classes.heroImgEl} />
        </div>
        <div ref={heroTxtRef} className={classes.heroLabel}>
          <h2 className={classes.heroLabelText}>{ts("label")}</h2>
        </div>
      </div>
      ...
      <div className={`container grid ${classes.grid}`}>
        <div className={classes.left}>
          <p className={classes.intro}>{ts("intro")}</p>
        </div>
        ...
        <div className={classes.subText}>
          <p className={classes.subTextP}>{ts("subText")}</p>
        </div>
      </div>
      ...
      <div className={`container grid ${classes.ctaGrid}`}>
        ...
        <div className={classes.ctaContent}>
          <p className={classes.ctaText}>{ts("ctaText")}</p>
          <Button href="/contact" variant="primary" size="lg">{t("checkAvailability")}</Button>
        </div>
      </div>
